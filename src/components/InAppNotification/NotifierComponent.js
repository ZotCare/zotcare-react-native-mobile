...
const NotifierComponent = React.memo(({ notification }) => {
	...
	return (
		...
		>
			...
			>
				<>
					<CustomAvatar name={user?.name} image={{ uri: user?.image?.small }} size={AVATAR_SIZE} fontSize={10}/> 
					<View style={styles.inner}>
						{/* <Text style={[styles.roomName, { color: Colors.themes[theme].titleText }]} numberOfLines={1}>{title}</Text> */}
						<Text style={[styles.message, { color: Colors.themes[theme].titleText }]} numberOfLines={1}>{text}</Text>
					</View>
				</>
			</Touchable>
			<Touchable
				...
				background={Touchable.SelectableBackgroundBorderless()}
			>
				<MaterialIcons name='close' style={[styles.close, { color: Colors.themes[theme].titleText }]} size={20} />
