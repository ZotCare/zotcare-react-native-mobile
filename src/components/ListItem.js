...
export default function ListItem({onPress, item}) {
  ...
  const TouchableCmt = Platform.select({
    ios: TouchableHighlight,
    android: TouchableNativeFeedback,
  });
  const background = Platform.select({
    ios: null,
    android: TouchableNativeFeedback.Ripple(Colors.underlayColor, false),
  });
  return (
        ...
        <View style={styles.surface2}>
          ...
          <View style={styles.detail}>
            ...
            <View style={styles.rowContent}>
              <Text
                style={[styles.content, Layout.font, briefStyle]}
                numberOfLines={1}>
                {item.brief}
              </Text>
            </View>
          ...
          <View style={{justifyContent: 'flex-start'}}>
            <Text style={[styles.content, Layout.font]}>
              {moment().diff(
