...
export const getAvatarSource = (avatar, token) => {
  ...
  let avatarSource = {};
  if (avatar == 'no_image' || avatar === undefined || avatar === '') {
    avatarSource = images.default_avatar;
  } else if (typeof avatar === 'number') {
    avatarSource = avatar;
  } else {
    avatarSource = {uri: avatar, headers};
  }
  return avatarSource;
};
...
const AvatarComponent = props => {
  ...
  if (props.verticalScale) {
    scaleFunction = verticalScale;
  }
  if (props.size) {
    size = scaleFunction(props.size);
  }
  ////console.log('AvatarComponent', avatarSource)
  return <Avatar.Image size={size} source={avatarSource} />;
};
