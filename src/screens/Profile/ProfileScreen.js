    ...
    <SafeAreaView style={[mainStyles.container]}>
      ...
      <ScrollView style={mainStyles.scrollViewContainer}>         
        ...
        <View style={{flexDirection: "row", justifyContent: "space-around", alignItems: "center", marginTop: verticalScale(30)}}>
          <Text style={[styles.bigText, {color: Colors.main_colors.purpleText, flex: 1, textAlign: "center"}]}>{profile.num_of_games ?? "0"}</Text>
          <Text style={[styles.bigText, {color: Colors.main_colors.purpleText, flex: 1, textAlign: "center"}]}>{!!profile.num_of_wins && !!profile.num_of_games && profile.num_of_games > 0 ? parseFloat(profile.num_of_wins/profile.num_of_games).toFixed(1)*100 + "%" : "0%"}</Text>
          <Text style={[styles.bigText, {color: Colors.main_colors.purpleText, flex: 1, textAlign: "center"}]}>{profile.score ?? "0"}</Text>
        </View>
      </ScrollView>          
    </SafeAreaView>
  )
