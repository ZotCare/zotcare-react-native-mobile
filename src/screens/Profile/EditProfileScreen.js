...
import { View, TouchableOpacity, BackHandler, SafeAreaView, ScrollView, Text } from 'react-native';
import { moderateScale, scale, ScaledSheet, verticalScale } from 'react-native-size-matters';
import { Header } from '../../components';
import Colors from "../../constants/Colors";
import { useDispatch, useSelector } from 'react-redux';
import { NavigationService } from '../../navigation'
import { getUUID, getProfile, getProfileSuccess } from '../../modules/profile/selectors';
import * as profileActions from '../../modules/profile/actions';
import CustomAvatar from '../../components/CustomAvatar';
import { Switch, Searchbar, IconButton } from "react-native-paper";
import { pickSingleForProfile, pickSingleWithCameraProfile } from '../../libs/imageApi'
  ...
  return (
    <SafeAreaView style={[mainStyles.authContainer]}>
      <ScrollView>
        ...
        <CustomModal visible={visible} onBackdropPress={() => setVisible(false)} component={imageSelectComponent()}/>
        <CustomModal visible={visible2} component={<Text style={{fontSize: 17, fontWeight: Platform.OS == "ios" ? '900' : "bold", color: Colors.main_colors.whiteText, textAlign: "center"}}>{"Profile updated successfully"}</Text>}/>
        <TouchableOpacity onPress={() => setVisible(true)} >
          <CustomAvatar name={profile.name} image={{ uri: profile.avatar }} size={120} fontSize={40}/>
          <Text style={[{textAlign: "center", paddingTop: verticalScale(5)}, mainStyles.textBold]}>Edit image</Text>
        </TouchableOpacity>
        <CustomTextInput
          value={name}
        ...
        <CustomButton
            ...
            style={{marginVertical: verticalScale(15), marginBottom: verticalScale(25)}}
          />
      ...
      <CustomModal 
        ...
        component={<Text style={{fontSize: 17, fontWeight: Platform.OS == "ios" ? '900' : "bold", color: Colors.main_colors.whiteText, textAlign: "center"}}>{"Are you sure you want to delete your account?"}</Text>} 
        whiteText={"No"} 
        pinkText={"Yes"}
        onPinkButtonPress={()=>{
